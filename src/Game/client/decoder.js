...
// Decoder.prototype.decode = function(data,schema){
...
// Decoder.prototype.countFields = function(schema){
...
// Decoder.prototype.decodeObject = function(pkg,offset,schema){
...
//     var nbProperties = this.countFields(schema);
...
//             if(self.isMaskTrue(propertiesMask,nbProperties,idx)) { // check the properties mask to see if the field is present in the object or not, and therefore has to be decoded or skipped
...
//             if(self.isMaskTrue(propertiesMask,nbProperties,idx)) {
...
//                 object[key] = self.decodeString(dv, length, offset);
...
//                             var res = this.decodeObject(pkg, offset, sc);
//                             result = res.object;
//                             offset = res.offset;
//                         }
...
//             if(this.isMaskTrue(propertiesMask,nbProperties,idx)) {
//                 //console.log('Decoding '+objName+' at offset '+offset);
//                 var res = this.decodeObject(pkg, offset, schema.standAlone[objName]);
...
//             if(this.isMaskTrue(propertiesMask,nbProperties,idx)) object[key] = !!this.isMaskTrue(bools,schema.booleans.length,boolidx); // !! converts to boolean
//             idx++;
//             boolidx++;
//         });
//     }
...
// Decoder.prototype.isMaskTrue = function(mask,nbProperties,idx){ // Process a bitmask to know if a specific field, at index idx, is present or not
//     return (mask >> (nbProperties-idx)) & 1; // Shift right to put the target at position 0, and AND it with 1
// };

// Decoder.prototype.decodeString = function(view,length,offset) { // Read length bytes starting at a specific offset to decode a string
//     var chars = [];
//     for(var i = 0; i < length; i++){
