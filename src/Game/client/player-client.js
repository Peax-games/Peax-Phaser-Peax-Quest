...
export default function Player(x,y,key){
...
Player.prototype = Object.create(Human);
...
Player.prototype.updateLife = function(){ // Update the life bar to reflect the amout of health of the player
    ...
    var tweenWidth = this.game.add.tween(Game.health.getChildAt(0)); // tween for the "body" of the bar
    var tweenEnd = this.game.add.tween(Game.health.getChildAt(1)); // tween for the curved tip
    ...
    if(door){
        ...
        if(this.isPlayer) {
            if (door.camera && !door.follow) { // if the camera cannot follow the player but has to be fixed at specific coordinates
                Game.unfollowPlayer();
                this.game.camera.x = door.camera.x;
                this.game.camera.y = door.camera.y;
...
Player.prototype.die = function(animate){
    ...
    if(this.isPlayer) {
        ...
        this.updateLife();
        setTimeout(Game.displayDeathScroll,window.Phaser.Timer.SECOND*2);
    }
