...
Game.prototype.makeIDmap = function (collection, map) {
...
Game.prototype.create = function () {
    var clio = new Client;
    this.HUD = window.game.add.group(); // Group containing all objects involved in the HUD
    this.HUD.add(window.game.add.sprite(0, 0, 'atlas1', 'border')); // Adds the gray border of the this.game
    var db = window.game.cache.getJSON('db');
    ...
    this.itemsInfo = db.items;
    this.npcInfo = db.npc;
    this.monstersInfo = db.monsters;
    ...
    this.entities = window.game.add.group(); // Group containing all the objects appearing on the map (npc, monster, items, players ...)
    this.scenery = window.game.add.group(); // Group containing all the animated sprites generated from the map
...
Game.prototype.updateWorld = function (data) { // data is the update package from the server
    var createdPlayers = [];
    if (data.newplayers) {
...
Game.prototype.populateTable = function (table, arr, callback) {
...
Game.prototype.traverseUpdateObject = function (obj, table, callback) {
...
Game.prototype.createMonster = function (data) { // data contains the data from the server on the new entity to create
...
Game.prototype.createItem = function (data) { // data contains the data from the server on the new entity to create
    var item;
...
Game.prototype.createPlayer = function (data) { // data contains the data from the server on the new entity to create
    var player;
    console.log(window.game)
...
var games = new Game();
Game.prototype.newPlayer = function (x, y, id) {
    // var player = games.create().playerFactory.next(x*this.map.tileWidth,y*this.map.tileHeight,'atlas3');
...
Game.prototype.setUpPlayer = function (player, data) { // data contains the data from the server on the new entity to create
    player.setName(data.name);
    player.speed = this.playerSpeed;
    player.orientation = this.defaultOrientation;
};

Game.prototype.fadeInTween = function (object) { // Fade-in effect used to spawn items and monsters
    object.alpha = 0;
...
Game.prototype.updatePlayerStatus = function (player, info) { // info contains the updated data from the server
...
Game.prototype.updateDisplayList = function () {
...
Game.prototype.updateEquipment = function (player, eqID) {
...
Game.prototype.updatePlayerAction = function (player, info) { // info contains the updated data from the server
...
Game.prototype.updateMonsterStatus = function (monster, info) { // info contains the updated data from the server
...
Game.prototype.updateMonsterAction = function (monster, info) { // info contains the updated data from the server
    if (info.alive == false && monster.alive == true) {
...
Game.prototype.updateItem = function (item, info) { // info contains the updated data from the server
...
Game.prototype.updateSelf = function (data) {
...
Game.prototype.revivePlayer = function () { // Revive the player after clicking "revive"
    Client.sendRevive();
...
Game.prototype.setLatency = function (latency) {
    this.latency = latency;
};

Game.prototype.initWorld = function (data, game) { // Initialize the this.game world based on the server data
    ...
    this.moveGroupTo(window.game.world, this.groundMapLayers, 0);
    this.moveGroupTo(window.game.world, this.scenery, this.groundMapLayers.z);
    this.moveGroupTo(window.game.world, this.markerGroup, this.scenery.z); // z start at 1
    this.moveGroupTo(window.game.world, this.entities, this.markerGroup.z);
    this.moveGroupTo(window.game.world, this.highMapLayers, this.entities.z);
    this.moveGroupTo(window.game.world, this.HUD, this.highMapLayers.z);
    ...
    //     this.game.stage.disableVisibilityChange = true; // Stay alive even if window loses focus
    // } else {
    //     this.game.onResume.addOnce(function () {
    ...
    // clio.emptyQueue(); // Process the queue of packets from the server that had to wait while the client was initializing
    ...
    // this.messageIn((this.isNewPlayer ? 'Welcome to window.PhaserQuest!' : 'Welcome back!'));

    if (this.isNewPlayer) this.toggleHelp();
};

Game.prototype.moveGroupTo = function (parent, group, endPos) {
...
// Game.prototype.displayHero = function(x,y,id){
//     this.player = this.newPlayer(x,y,id);
//     this.player.setIsPlayer(true);
//     this.player.addChild(this.cameraFocus = this.game.add.sprite(0, 16)); // trick to force camera offset
//     this.followPlayer();
// };

// MOVE CODE

Game.prototype.moveCharacter = function (id, end, orientation, delta) { // Move character according to information from the server
...
Game.prototype.moveMonster = function (id, path, delta) { // Move monster according to information from the server
...
Game.prototype.removePlayer = function (player, animate) {
...
Game.prototype.makeFlatScroll = function (callback) { // Creates and empty, generic flat scroll screen, to be used for achievements and help
    // callback is the function to call when clicking on the close button (typically a toggle function, such as toggleHelp() )
    var scroll = this.game.add.sprite(80, 32, 'atlas1', 'achievements');
    scroll.fixedToCamera = true;
    scroll.alpha = 0;
    scroll.visible = false;
    Home.setFadeTweens(scroll);
    var closeBtn = scroll.addChild(this.game.add.button(scroll.width - 18, -14, 'atlas1', callback, this, 'close_1', 'close_0', 'close_2'));
    closeBtn.input.useHandCursor = false;
    return scroll;
};

Game.prototype.makeHelpScroll = function () { // Make the screen showing how to play instructions
    ...
    var mouse = this.helpScroll.addChild(this.game.add.sprite(55, mouseY, 'atlas1', 'mouse'));
    mouse.anchor.set(0.5);
    this.helpScroll.addChild(this.game.add.text(100, mouseY - 10, this.db.texts.help_move, style));
    var enter = this.helpScroll.addChild(this.game.add.sprite(55, enterY, 'atlas1', 'enter'));
    enter.anchor.set(0.5);
    this.helpScroll.addChild(this.game.add.text(100, enterY - 12, this.db.texts.help_chat, style));
    var char = this.helpScroll.addChild(this.game.add.sprite(55, charY, 'atlas3', 'clotharmor_31'));
    char.anchor.set(0.5);
    this.helpScroll.addChild(this.game.add.text(100, charY - 10, this.db.texts.help_save, style));
};
...
Game.prototype.toggleHelp = function () { // Toggles the visibility state of the help screen
...
Game.prototype.computeTileCoords = function (x, y) {
...
Game.prototype.computeView = function () {
    this.view = new window.Phaser.Rectangle(this.game.camera.x + this.borderPadding, this.game.camera.y + this.borderPadding,
        this.game.camera.width - this.borderPadding * 2, this.game.camera.height - this.borderPadding * 2 - this.HUDheight);
};
...
Game.prototype.followPlayer = function () { // Make the camera follow the player, within the appropriate bounds
    this.inDoor = false;
    // Rectangle to which the camera is bound, cannot move outside it
    var width = (this.player.x >= 92 ? 113 : 92);
    this.game.camera.bounds = new window.Phaser.Rectangle(this.map.tileWidth - this.borderPadding, this.map.tileWidth - this.borderPadding, width * this.map.tileWidth, 311 * this.map.tileWidth);
    this.game.camera.follow(this.cameraFocus);
    this.cameraFollowing = true;
};

Game.prototype.followPlayerIndoors = function (x, y, mx, my) { // Follow player but with extra constraints due to being indoors
...
Game.prototype.unfollowPlayer = function () { // Make the camera stop following player, typically because he is in a small indoors area
    this.inDoor = true;
...
Game.prototype.basicAnimation = function (sprite) { // sprite is the sprite to which the animation should be applied
    var frames = [];
...
Game.prototype.basicAtlasAnimation = function (sprite) { // sprite is the sprite to which the animation should be applied
...
Game.prototype.displayHUD = function (game) {
    var lifeX = this.borderPadding;
    var lifeY = window.game.height - this.borderPadding - this.HUDheight + 6;
    this.barY = window.game.height - this.borderPadding - this.HUDheight;
    
    this.HUDbuttons = window.game.add.group();

    // this.displayChatBar();
    // this.displayAchievementDock();

    // this.HUD.add(this.game.add.sprite(this.borderPadding, this.barY, 'atlas1', 'bar'));
    // this.HUD.add(this.weaponIcon = window.game.add.sprite(this.borderPadding + 210, this.barY, 'atlas3'));
    // this.HUD.add(this.armorIcon = window.game.add.sprite(this.borderPadding + 244, this.barY + 3, 'atlas3'));

    // this.HUDmessage = null;
    // this.messages = window.game.add.group();
    // for (var m = 0; m < 4; m++) {
    //     this.messages.add(window.game.add.text(490, this.barY + 5, '', {
    //         font: '16px pixel',
    //         fill: "#eeeeee"
    //     }));
    // }
    ...
    // this.nbConnectedText = this.HUD.add(window.game.add.text(745, this.barY + 8, '0 players', {
    //     font: '16px pixel',
    //     fill: "#eeeeee"
    // }));

    // this.chatButton = this.HUDbuttons.add(this.game.add.button(850, this.barY + 2, 'atlas1', this.toggleChat, this, 'talkicon_1', 'talkicon_0', 'talkicon_2'));
    // this.achButton = this.HUDbuttons.add(this.game.add.button(880, this.barY + 2, 'atlas1', this.toggleAchievements, this, 'achievementicon_1', 'achievementicon_0', 'achievementicon_2'));
    // this.helpButton = this.HUDbuttons.add(this.game.add.button(910, this.barY + 2, 'atlas1', this.toggleHelp, this, 'helpicon_1', 'helpicon_0', 'helpicon_2'));
    // this.HUDbuttons.add(this.game.add.button(940, this.barY + 2, 'atlas1', function (_btn) {
    ...
    // this.achTween = this.game.add.tween(this.achButton);
    ...
    // this.HUD.add(this.game.add.sprite(lifeX, lifeY, 'atlas1', 'life'));
    ...
    // var chatKey = this.game.input.keyboard.addKey(window.Phaser.Keyboard.ENTER);
...
// Game.prototype.computeLifeBarWidth = function () {
...
// Game.prototype.createLifeBar = function (lifeX, lifeY) {
...
//     this.health = this.game.add.sprite(lifeX + 20, lifeY + 4);
//     this.health.addChild(this.game.add.tileSprite(0, 0, width, 18, 'life', 0));
//     this.health.addChild(this.game.add.sprite(width, 0, 'life', 1));
// };

Game.prototype.createMarker = function () { // Creates the white marker that follows the pointer
    this.markerGroup = this.game.add.group();
    this.marker = this.markerGroup.add(this.game.add.sprite(0, 0, 'atlas1'));
...
Game.prototype.updateMarker = function (x, y, collide) { // Makes the marker white or red depending on whether the underlying tile is collidable
...
Game.prototype.messageIn = function (txt) { // Slide a message in the message area of the HUD
    ...
    var yTween = this.game.add.tween(msg.cameraOffset);
    var alphaTween = this.game.add.tween(msg);
    ...
    this.HUDmessage = msg;
    var outTween = this.game.add.tween(msg);
    outTween.to({}, window.Phaser.Timer.SECOND * 3);
    outTween.onComplete.add(this.messageOut, this);
    outTween.start();
};

Game.prototype.messageOut = function (msg) { // Slide a message in the message area of the HUD
...
Game.prototype.toggleChat = function () { // Toggles the visibility of the chat bar
...
Game.prototype.updateNbConnected = function (nb) {
...
Game.prototype.displayMap = function () {
    this.groundMapLayers = this.game.add.group();
    this.highMapLayers = this.game.add.group();
    ...
    for (var i = 0; i < this.map.layers.length; i++) {
        ...
        this.map.gameLayers[i].visible = false; // Make map invisible before the this.game has fully loaded
    }
...
Game.prototype.createCollisionArray = function () {
...
Game.prototype.createDoorsMap = function () { // Create the associative array mapping coordinates to doors/teleports
...
Game.prototype.displayScenery = function () {
...
Game.prototype.displayNPC = function () {
    var entities = this.game.cache.getJSON('entities'); // mapping from object IDs to sprites, the sprites being keys for the appropriate json file
...
Game.prototype.enableClick = function () {
    this.clickEnabled = true;
};

Game.prototype.disableClick = function () {
...
Game.prototype.handleCharClick = function (character) { // Handles what happens when clicking on an NPC
    if (this.handleClick()) {
...
Game.prototype.handleChestClick = function (chest) { // Handles what happens when clicking on a chest
    if (this.handleClick()) {
...
Game.prototype.handleLootClick = function (loot) { // Handles what happens when clicking on an item
    if (this.handleClick()) {
...
Game.prototype.handleMapClick = function (layer, pointer) { // Handles what happens when clicking on an empty tile to move
    if (this.handleClick()) {
...
Game.prototype.handleMonsterClick = function (monster) { // Handles what happens when clicking on a monster
    if (this.handleClick()) {
...
Game.prototype.manageMoveTarget = function (x, y) {
...
Game.prototype.setHoverCursors = function (sprite, cursor) { // Sets the appearance of the mouse cursor when hovering a specific sprite
...
Game.prototype.resetHoverCursors = function (sprite) {
...
Game.prototype.makeHPtexts = function () { // Create a pool of HP texts to (re)use when needed during the this.game
    this.HPGroup = this.game.add.group();
    for (var b = 0; b < 60; b++) {
        this.HPGroup.add(this.game.add.text(0, 0, '', {
            font: '20px pixel',
            strokeThickness: 2
        }));
    }
...
Game.prototype.displayHP = function (txt, color, target, delay) { // Display hit points above a sprite
...
Game.prototype.playerSays = function (id, txt) {
...
Game.prototype.makeBubble = function () { // Create a speech bubble
...
Game.prototype.markerHasMoved = function () {
    return (this.previousMarkerPosition.x != this.markerPosition.x || this.previousMarkerPosition.y != this.markerPosition.y);
};

Game.prototype.sortEntities = function () { // Sort the members of the "entities" group according to their y value, so that they overlap nicely
...
Game.prototype.update = function () { // Main update loop of the client
    ...
    var cell = this.computeTileCoords(this.game.input.activePointer.worldX, this.game.input.activePointer.worldY);
