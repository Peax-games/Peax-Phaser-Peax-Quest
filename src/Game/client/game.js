...
//     this.load.tilemap('map', 'assets/maps/minimap_client.json', null, window.Phaser.Tilemap.TILED_JSON);
//     this.load.spritesheet('tileset', 'assets/tilesets/tilesheet.png',32,32);
//     this.load.atlasJSONHash('atlas4', 'assets/sprites/atlas4.png', 'assets/sprites/atlas4.json'); // Atlas of monsters
//     this.load.spritesheet('bubble', 'assets/sprites/bubble2.png',5,5); // tilesprite used to make speech bubbles
//     this.load.spritesheet('life', 'assets/sprites/lifelvl.png',5,18); // tilesprite used to make lifebar
//     this.load.audio('sounds','assets/audio/sounds.mp3','assets/audio/sounds.ogg'); // audio sprite of all sound effects
//     this.load.json('entities', 'assets/json/entities_client.json'); // Basically a list of the NPC, mapping their id to the key used in other JSON files
// };
...
this.makeIDmap = function(collection,map){
...
this.updateWorld = function(data) { // data is the update package from the server
    var createdPlayers = [];
    if(data.newplayers) {
...
this.populateTable = function(table,arr,callback){
...
this.traverseUpdateObject = function(obj,table,callback){
...
this.createMonster = function(data){ // data contains the data from the server on the new entity to create
...
this.createItem = function(data) { // data contains the data from the server on the new entity to create
    var item;
...
this.createPlayer = function(data){ // data contains the data from the server on the new entity to create
    var player;
...
this.newPlayer = function(x,y,id){
...
this.setUpPlayer = function(player,data){ // data contains the data from the server on the new entity to create
    player.setName(data.name);
    player.speed = this.playerSpeed;
    player.orientation = this.defaultOrientation;
};

this.fadeInTween = function(object){ // Fade-in effect used to spawn items and monsters
    object.alpha = 0;
...
this.updatePlayerStatus = function(player,info){ // info contains the updated data from the server
...
this.updateEquipment = function(player,eqID){
...
this.updatePlayerAction = function(player,info){ // info contains the updated data from the server
...
this.updateMonsterStatus = function(monster,info){ // info contains the updated data from the server
...
this.updateMonsterAction = function(monster,info){ // info contains the updated data from the server
    if(info.alive == false && monster.alive == true){
...
this.updateItem = function(item,info){ // info contains the updated data from the server
...
this.revivePlayer = function(){ // Revive the player after clicking "revive"
    Client.sendRevive();
...
this.initWorld = function(data){ // Initialize the this world based on the server data
...
this.moveGroupTo = function(parent,group,endPos){
...
this.displayHero = function(x,y,id){
...
this.moveCharacter = function(id,end,orientation,delta){ // Move character according to information from the server
...
this.moveMonster = function(id,path,delta){ // Move monster according to information from the server
...
this.removePlayer = function(player,animate){
...
this.makeAchievementsScroll = function(){ // Create the screen displaying the achievements of the player
...
this.makeDeathScroll = function(){ // Make the screen that is displayed when player dies
...
this.makeFlatScroll = function(callback){ // Creates and empty, generic flat scroll screen, to be used for achievements and help
...
this.makeHelpScroll = function(){ // Make the screen showing how to play instructions
...
this.makeOrientationScreen = function(){
...
this.displayDeathScroll = function(){ // Displayed when player dies
...
this.displayOrientationScreen = function(){
...
this.removeOrientationScreen = function(){
    this.orientationContainer.visible = false;
};

this.toggleHelp = function(){ // Toggles the visibility state of the help screen
...
this.toggleAchievements = function(){ // Toggles the visibility state of the achievements screen
...
this.changeAchievementsPage = function(dir){
...
this.updateAchievementsArrows = function(){
...
this.handleLootAchievement = function(id){ // item id
...
this.handleKillAchievement = function(id){ // monster id
...
this.handleLocationAchievements = function(){
...
this.getAchievement = function(id){ // achievement id
...
this.findLocationAchievements = function(){
...
this.detectElement = function(map,x,y){
...
this.computeFinalOrientation = function(path){ // path is a list of cells
...
this.computeTileCoords = function(x,y){
...
this.checkCameraBounds = function(){
...
this.tweenCameraBounds = function(width){
...
this.followPlayer = function(){ // Make the camera follow the player, within the appropriate bounds
    this.inDoor = false;
...
this.followPlayerIndoors = function(x,y,mx,my){ // Follow player but with extra constraints due to being indoors
...
this.unfollowPlayer = function(){ // Make the camera stop following player, typically because he is in a small indoors area
    this.inDoor = true;
...
this.basicAnimation = function(sprite){ // sprite is the sprite to which the animation should be applied
    var frames = [];
...
this.basicAtlasAnimation = function(sprite){ // sprite is the sprite to which the animation should be applied
...
this.displayAchievementDock = function(){
...
this.computeLifeBarWidth = function(){
...
this.createLifeBar = function(lifeX,lifeY){
...
this.createMarker = function(){ // Creates the white marker that follows the pointer
...
this.updateMarker = function(x,y,collide) { // Makes the marker white or red depending on whether the underlying tile is collidable
...
this.messageIn = function(txt){ // Slide a message in the message area of the HUD
...
this.messageOut = function(msg){ // Slide a message in the message area of the HUD
...
this.toggleChat = function(){ // Toggles the visibility of the chat bar
...
this.updateNbConnected = function(nb){
...
this.createCollisionArray = function(){
...
this.createDoorsMap = function(){ // Create the associative array mapping coordinates to doors/teleports
...
this.handleCharClick = function(character){ // Handles what happens when clicking on an NPC
    if (this.handleClick()) {
...
this.handleChestClick = function(chest){ // Handles what happens when clicking on a chest
    if (this.handleClick()) {
...
this.handleLootClick = function(loot){ // Handles what happens when clicking on an item
    if (this.handleClick()) {
...
this.handleMapClick = function(layer,pointer){ // Handles what happens when clicking on an empty tile to move
    if (this.handleClick()) {
...
this.handleMonsterClick = function(monster){ // Handles what happens when clicking on a monster
    if (this.handleClick()) {
...
this.manageMoveTarget = function(x,y){
...
this.setHoverCursors = function(sprite,cursor){ // Sets the appearance of the mouse cursor when hovering a specific sprite
...
this.resetHoverCursors = function(sprite){
...
this.makeHPtexts = function(){ // Create a pool of HP texts to (re)use when needed during the this
...
this.displayHP = function(txt,color,target,delay){ // Display hit points above a sprite
...
this.playerSays = function(id,txt){
...
this.makeBubble = function(){ // Create a speech bubble
...
this.markerHasMoved = function(){
    return (this.previousMarkerPosition.x != this.markerPosition.x || this.previousMarkerPosition.y != this.markerPosition.y);
};

this.sortEntities = function(){ // Sort the members of the "entities" group according to their y value, so that they overlap nicely
    this.entities.sort('y', window.Phaser.Group.SORT_ASCENDING);
};

this.update = function(){ // Main update loop of the client
...
this.render = function(){ // Use to display debug information, not used in production
